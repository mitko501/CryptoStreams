...
namespace block {
    ...
    void piccolo::Encrypt(uint8_t *block) {
        ...
        *x1 = *x1 ^ F(*x0) ^ READ_ROUND_KEY_WORD(rk[2*_rounds - 2]);
        *x3 = *x3 ^ F(*x2) ^ READ_ROUND_KEY_WORD(rk[2*_rounds - 1]);
    ...
    void piccolo::Decrypt(uint8_t *block) {
        ...
        *x0 ^= READ_ROUND_KEY_WORD(rk[52]);
        for (i = 0; i < _rounds - 1; ++i) {
            if ((i & 0x01) == 0) {
                *x1 = *x1 ^ F(*x0) ^ READ_ROUND_KEY_WORD(rk[2 * _rounds - 2 * i - 2]);
                *x3 = *x3 ^ F(*x2) ^ READ_ROUND_KEY_WORD(rk[2 * _rounds - 2 * i - 1]);
            } else {
                *x1 = *x1 ^ F(*x0) ^ READ_ROUND_KEY_WORD(rk[2 * _rounds - 2 * i - 1]);
                *x3 = *x3 ^ F(*x2) ^ READ_ROUND_KEY_WORD(rk[2 * _rounds - 2 * i - 2]);
            }
            RP(x0, x1, x2, x3);
        }
