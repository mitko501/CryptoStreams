...
namespace block {
    ...
    class rectangle : public lightweight<RECTANGLE_ROUND_KEYS_SIZE, RECTANGLE_BLOCK_SIZE> {
        ...
        void key_schedule_80(const std::uint8_t *key);
        void key_schedule_128(const std::uint8_t *key);
    ...
    public:
        ...
        void keysetup(const std::uint8_t *key, const std::uint64_t keysize) override {
            if (keysize == RECTANGLE_KEY_SIZE_80) {
                key_schedule_80(key);
            } else if(keysize == RECTANGLE_KEY_SIZE_128) {
                key_schedule_128(key);
            } else {
                throw std::runtime_error("RECTANGLE function only support key sizes: "
                                         + std::to_string(RECTANGLE_KEY_SIZE_80) + ", "
                                         + std::to_string(RECTANGLE_KEY_SIZE_128));
            }
        }
